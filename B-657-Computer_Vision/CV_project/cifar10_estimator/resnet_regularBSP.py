...
def get_model_fn(num_gpus, variable_strategy, num_workers, gradient_scale):
  ...
  def _resnet_model_fn(features, labels, mode, params):
    ...
    os.environ['tensor_for_global_grad_norm'] = 'gradient_global_norm:0'
    os.environ['tensor_for_expected_gradient_norm'] = 'estimated_gradient_norm:0'
    os.environ['tensor_for_b_simple2'] = 'b_simple2:0'
    os.environ['tensor_for_expected_gradient_norm2'] = 'estimated_gradient_norm2:0'

    os.environ['tensor_for_b_simple_opt'] = 'resnet/tower_0/b_simple_opt:0'
    os.environ['tensor_for_expected_gradient_opt_norm'] = 'resnet/tower_0/estimated_gradient_opt_norm:0'
    ...
    with tf.device(consolidation_device):
      ...
      optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=momentum, use_locking=True)
      ...
      tf.logging.info('@sahiltyagi4 sync_replica ################### ' + str(params.is_chief))
    ...
    with tf.device(consolidation_device):
      ...
      grad_noise_tensors = {'global_grad_norm_per_worker': gradient_global_norm, 
                            'worker_global_step': tf.train.get_global_step()}
    
      #overall_gradient_variance = tf.math.reduce_variance(flattened_gradients)

      # grad_noise_tensors = {'global_grad_norm_per_worker': gradient_global_norm, 
      #                       'worker_global_step': tf.train.get_global_step(),
      #                       'overall_gradient_variance': overall_gradient_variance}
      ...
      # grad_noise_tensors = {'expected_grad_norm_per_worker': estimated_gradient_norm_opt_val, 
      #                       'b_simple_per_worker': B_simple_opt, 
      #                       'global_grad_norm_per_worker': gradient_global_norm, 
      #                       'worker_global_step': tf.train.get_global_step(),
      #                       'overall_gradient_variance': overall_gradient_variance}
      
      noise_logging_hook = tf.train.LoggingTensorHook(tensors=grad_noise_tensors, every_n_iter=1)
      train_hooks.append(noise_logging_hook)
    ...
    # asp_adjust_strategy can be 'staleness' or 'iteration_times'
...
def main(job_dir, data_dir, num_gpus, variable_strategy,
  ...
  # run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=job_dir, 
  #   save_checkpoints_steps=500, node_batch_size=node_batch_size, keep_checkpoint_max=7, switched_input_fn=input_fn, 
  #   data_dir='/resnet-cifar10/models/tutorials/image/cifar10_estimator/cifar-10-data', experimental_distribute=strategy)

  run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=job_dir, 
    save_checkpoints_steps=400, workload='resnet', node_batch_size=node_batch_size, keep_checkpoint_max=7, switched_input_fn=input_fn, 
