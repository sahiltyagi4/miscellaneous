  ...
  def _resnet_model_fn(features, labels, mode, params):
    ...
    with tf.device(consolidation_device):
      ...
      optimizer = tf.train.MomentumOptimizer(
          learning_rate=learning_rate, momentum=momentum, use_locking=True)
      train_hooks = []
...
def main(job_dir, data_dir, num_gpus, variable_strategy,
  ...
  run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=job_dir, 
    save_checkpoints_steps=500, workload='resnet', node_batch_size=node_batch_size, keep_checkpoint_max=7, switched_input_fn=input_fn, 
