        ...
        with tf.name_scope('tower_%d' % i) as name_scope:
          with tf.device(device_setter):
            loss, gradvars, preds, compgrad_op = _tower_fn(
                ...
                params.batch_norm_epsilon, optimizer, gradient_scale, w_name=w_name)
...
def _tower_fn(is_training, weight_decay, feature, label, data_format,
              num_layers, batch_norm_decay, batch_norm_epsilon, optimizer, gradient_scale, w_name):
...
def main(job_dir, data_dir, num_gpus, variable_strategy,
  ...
  tasktype = tf_config['task']['type']
  index = tf_config['task']['index']
  node_batch_size = batchlist[index]

  # mirrored_strategy = tf.distribute.MirroredStrategy()
  multi_mirrored_strategy = tf.compat.v1.distribute.experimental.MultiWorkerMirroredStrategy()
  run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=job_dir, node_batch_size=node_batch_size,
    save_checkpoints_steps=500, train_distribute=multi_mirrored_strategy)
  ...
  if warm_start == 'true':
    ...
    classifier = tf.estimator.Estimator(model_fn=get_model_fn(num_gpus,variable_strategy,run_config.num_worker_replicas or 1, gradient_scale),
      config=run_config,params=hparams, warm_start_from=warmstart_obj)
