...
def get_model_fn(num_gpus, variable_strategy, num_workers, gradient_scale):
  ...
  def _resnet_model_fn(features, labels, mode, params):
    ...
    tf_config = json.loads(os.environ['BATCH_LIST'])
...
def _tower_fn(is_training, weight_decay, feature, label, data_format,
  ...
  for new_g, new_v in zip(new_grads, model_params):
    tf.summary.histogram(new_v.name + '_MODIFIED', new_g)
...
def main(job_dir, data_dir, num_gpus, variable_strategy,
  ...
  try:
    os.environ['UNIFORM_CLUSTER_BATCH_SIZE'] = '384'
    # os.environ['BATCH_LIST'] = '{\"batch_size_list\": [110,220,54]}'
    os.environ['BATCH_LIST'] = '{\"batch_size_list\": [128,128,128]}'
  ...
  hvd.init()
  tf.logging.info('********************************************************************* hvd v-gpu2 rank is: ' + str(hvd.rank()))
  tf.logging.info('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ hvd size is: ' + str(hvd.size()))
  ...
  tf_config = json.loads(os.environ['BATCH_LIST'])
  batchlist = tf_config['batch_size_list']
  node_batch_size = batchlist[hvd.rank()]
  tf.logging.info('$$$$$$$$$$$$$$$$$$$$$$$$$ node_batch_size on node v-gpu2 is ' + str(node_batch_size))

  model_dir = job_dir if hvd.rank() == 0 else None
  #model_dir = job_dir
  
  run_config = tf.estimator.RunConfig(session_config=sess_config, model_dir=model_dir, save_checkpoints_steps=500, 
    node_batch_size=node_batch_size, keep_checkpoint_max=7, mpi_rank=hvd.rank())
  ...
  average_batchsize = (batchsum/(len(batchlist)))
